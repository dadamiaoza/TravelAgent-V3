...
## 9. 下一步：常驻 AI 对话 + 地图/列表联动 + 长短期记忆

> 详细技术方案见：[ai-chat-collaboration-design.md](./ai-chat-collaboration-design.md)

### 计划顺序

```text
A. 点位编号 + 列表/地图双向聚焦
B. Zustand 统一数据源 + 编辑/排序实时联动 + 后端增删/geocode/reoptimize
C1. 常驻对话面板 + 结构化建议卡片 + 采纳落地
C2. SSE 流式回复
D. 长短期记忆 + 上下文摘要
```
...
- AI 不直接改库，先给建议卡片，用户采纳后再写回
- 用户手动编辑优先于 AI 建议
- 地图、列表、对话使用同一份客户端 store
- 短期记忆沿用 PostgresSaver，长期记忆后续可加偏好/记忆表
