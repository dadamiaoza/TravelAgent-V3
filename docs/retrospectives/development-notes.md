...
### 曾尝试
- 曾尝试跳过 route_optimizer Agent，直接调用工具函数来提速。

### 回退
- 已回退该方案，恢复使用 route_optimizer Agent。
- 原因：
  1. route_optimizer Agent 是原有编排链的一环，负责在 LLM 链中调用路线优化工具。
  2. 保持 Agent 链路可以让后续扩展“LLM 对路线优化做决策/重试/修复”保留入口。
  3. 跳过 Agent 虽然减少一次 LLM 调用，但破坏了原有的多 Agent 编排一致性。
...
## 28. 减少全量 N×N 高德调用（不破坏 Agent 链路）

### 方案
- 保留 route_optimizer Agent 链路。
- 在工具内部优化：
