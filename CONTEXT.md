...
**行程** (Itinerary):
系统生成的、按"天-时段-地点"组织的完整**旅行计划**。由若干行程节点组成。是规划与编辑的真源；照片不会覆盖它的顺序和节点集合。
_Avoid_: 攻略、方案、真实行程
...
**实际到访** (Visit Stop):
根据照片 GPS 聚类、经用户确认后的真实停留。挂在某次行程上，**不**写入计划行程的节点列表。离某计划节点足够近时可关联该节点，不必另开钉。
_Avoid_: 真实行程、实际行程、补点、打卡点

**回忆** (Recollection):
展示实际足迹的视图（路由可仍为 `/map`）。计划节点作淡底，已确认实际到访作实心钉。
_Avoid_: 地图 Tab、照片 Tab、第二份行程
...
- 一个**行程**包含若干**行程节点**
- 一次行程还可以有若干**实际到访**（用户确认后的照片簇）；二者不是两份可编辑 Day 列表
- **回忆**读取计划（淡底）和已确认实际到访（实心钉）
...
> **Dev:** "用户没按计划走，照片对不上节点，要不要自动往行程里加点？"
> **领域专家:** "不要静默改**行程**。GPS 聚类给出建议，用户确认后写成**实际到访**，在**回忆**里看。计划节点列表不变。"
>
...
- "真实行程 / 实际行程" 容易被做成第二份可编辑 Day 列表 —— 应使用**实际到访**（确认后的停留）和**回忆**（视图），不要覆盖**行程**。
...
- **计划 vs 实际**：行程 Tab 编辑计划；回忆 Tab 看确认后的实际到访。照片不静默改 `itinerary_items`。
