...
def _parse_sse(text: str) -> dict[str, dict]:
    ...
    for name, payload in _iter_sse(text):
        events[name] = payload
    return events


def _iter_sse(text: str) -> list[tuple[str, dict]]:
...
def test_chat_stream_emits_applied_event() -> None:
    ...
    assert events["status"]["status"] == "thinking"


def test_chat_stream_emits_tool_status() -> None:
    trip_id = uuid4()
    result = TripChatOut(reply="明天小雨", thread_id=f"trip-chat-{trip_id}", suggestions=[], applied=[])

    def fake_run(*, progress=None, **_kwargs):
        if progress:
            progress("check_facts", "正在查询天气和开放时间…")
        return result
    ...
    app.dependency_overrides[get_db] = lambda: db
    try:
        with patch("app.api.v1.trips.run_trip_chat", side_effect=fake_run):
            response = TestClient(app).post(
                f"/api/v1/trips/{trip_id}/chat/stream",
                json={"message": "第二天会下雨吗"},
            )
    finally:
        if original is not None:
            app.dependency_overrides[get_db] = original
    ...
    assert response.status_code == 200
    payloads = [payload for name, payload in _iter_sse(response.text) if name == "status"]
    messages = [item.get("message") for item in payloads]
    assert "AI 正在思考…" in messages
    assert "正在查询天气和开放时间…" in messages
