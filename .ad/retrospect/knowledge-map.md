...
- [17. LLM 传输层：brotli 500 与 MiniMax M3](#17-llm-传输层brotli-500-与-minimax-m3)
- [18. 照片上传 422 与同名路地理编码](#18-照片上传-422-与同名路地理编码)
...
| §17 | 2026-09-11：LLM HTTP 与 M3 | 不要把 suggest 500 当成「没代理」 |
| §18 | 2026-09-12：上传 422 与同名路 | 不要把 422 当成格式错误；不要靠全国搜消同名歧义 |
...
学习文档里的坑，只有写成「现象 → 误判 → 原则 → 下一问」才能带面试节奏。手法和题库放在本地 `.inter/`（不入库）。

主钩子只用一个：进度条停在 30%。可靠任务是「后来还遇到」的第二问，不要开场并讲。

---

## 17. LLM 传输层：brotli 500 与 MiniMax M3

完整因果见 [提示词优化 500 + M3 升级](2026-09-11_Suggest-500-and-M3.md)。这里只留判断句。

**首页优化提示词 500，日志写 Connection error。** 先看是不是响应体解压挂了，不要先改代理。

openai 3 + httpx2 + MiniMax 的 `br` + Anaconda brotli 1.0.9 = `process() takes no keyword arguments`。应用层收口：`app.core.llm.chat_model()`，`Accept-Encoding: gzip, deflate`。

当前默认模型是 **MiniMax-M3**（`LLM_MODEL` / `Settings.llm_model`）。所有 `ChatOpenAI` 必须经工厂创建。思考链仍在 `content` 的 `<think>` 里剥；不要为了干净 JSON 去开 `reasoning_split`，会伤 Agent 多轮 tool。

---

## 18. 照片上传 422 与同名路地理编码

完整因果见 [批量上传 422 + 黄兴路定位](2026-09-12_Photo-Upload-422-and-Huangxing-Geocode.md)。

**批量上传 422。** 先看是不是请求体校验（空 `item_id`、文件字段名、`FormData` 被标成 JSON），不要先查 EXIF。上传走 `request.form()`，空 UUID 当没传。

**长沙的黄兴路步行街钉在上海。** 高德 `city` 不是硬限制；「黄兴路」是查询的子串，分会高于「黄兴南路步行街」。有城市就要 `citylimit` + 过滤，禁止全国搜兜底。跨城点写 POI 自己的 `city`。
